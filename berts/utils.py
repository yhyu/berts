...
def get_bert_inputs(tokenizer, seq1, seq2=None):
    # encode sentence and preppend CLS, and padding (tf.ragged)
    sentence1 = tf.ragged.constant([encode_sentence(tokenizer, s) for s in seq1])
    ...
    if seq2 != None:
        sentence2 = tf.ragged.constant([encode_sentence(tokenizer, s) for s in seq2])
